...
+ (NSDate *)zhuanghuaShiqu:(NSDate *)oldDate
{
    NSTimeZone *zone = [NSTimeZone systemTimeZone];
    NSInteger interval = [zone secondsFromGMTForDate: oldDate];
    return [oldDate  dateByAddingTimeInterval: interval];
}

+ (NSInteger)numberOfDaysWithFromDate:(NSDate *)fromDate toDate:(NSDate *)toDate{
         NSCalendar *calendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    
         NSDateComponents    * comp = [calendar components:NSCalendarUnitDay| NSCalendarUnitHour
                                                                                       fromDate:fromDate
                                                                                        toDate:toDate
                                                                                       options:NSCalendarWrapComponents];
    NSInteger tianshu=comp.day;
    if (comp.hour>=1) {//超过一小时多算一天
        tianshu+=1;
    }
         return tianshu;
...
+ (BOOL)dateWhoBig:(NSDate *)oneDate twoDate:(NSDate *)twoDate
{
    NSTimeInterval oneInt = [oneDate timeIntervalSinceDate:twoDate];
    if (oneInt>0) {
        return NO;
    }else{
        return YES;
