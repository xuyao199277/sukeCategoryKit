...
+ (UIImage *)reSizeImage:(UIImage *)image toWidthOrHeight:(CGFloat)length
{
    CGRect frame;
    //比例不变
    if (image.size.width>=image.size.height) {
        frame=CGRectMake(0, 0, length, length*image.size.height/image.size.width);
    }else{
        frame=CGRectMake(0, 0, length*image.size.width/image.size.height, length);
    }
